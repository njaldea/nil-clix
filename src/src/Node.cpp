...
namespace nil::clix
{
    ...
    void Node::runner(int (*init_exec)(const nil::clix::Options&))
    {
        exec = init_exec;
    }
    ...
    void Node::add(std::string key, std::string description, void (*sub_noder)(Node&))
    {
        if (find(key) != nullptr)
        {
        ...
        sub.emplace_back(SubNode(std::move(key), std::move(description), sub_noder));
    }
