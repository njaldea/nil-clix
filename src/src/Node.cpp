...
#include <nil/clix/Command.hpp>
#include <nil/clix/OptionInfo.hpp>
#include <nil/clix/Options.hpp>

#include <stdexcept>

namespace nil::clix {
Node Node::root() { return Node{std::make_unique<Command>()}; }
...
Node &Node::add(std::string key, std::string description,
                std::unique_ptr<Command> sub_command) {
  if (find(key) != nullptr) {
    throw std::invalid_argument("[nil][cli]: option \"" + key +
                                "\" already exists");
  }

  SubNode subnode{std::move(key), std::move(description),
...
const Node *Node::find(std::string_view name) const {
  auto result =
      std::find_if(std::begin(sub), std::end(sub),
