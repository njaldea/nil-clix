...
struct Options::Impl final {
  explicit Impl(std::string init_usage)
      : usage(std::move(init_usage)), desc("OPTIONS") {}
  ...
  template <typename T> auto access(const std::string &k) {
    try {
      return vm[k].as<T>();
...
Options::Options(const OptionInfo &info, std::string usage,
                 const std::vector<SubNode> &subnodes, int argc,
