...
Builder &Builder::flag(std::string lkey, conf::Flag options) {
  ...
  public:
    Option(std::string init_lkey, conf::Flag init_options)
        : lkey(std::move(init_lkey)), options(std::move(init_options)) {}
  ...
  info.emplace_back(
      std::make_unique<Option>(std::move(lkey), std::move(options)));
  return *this;
}
  ...
  public:
    ...
    void apply(const IOption::Impl &impl) const override {
      ...
      if (options.fallback.has_value()) {
        value->default_value(options.fallback.value(),
...
OptionInfo Builder::build() { return std::move(info); }
} // namespace nil::clix
